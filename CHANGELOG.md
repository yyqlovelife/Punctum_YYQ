...
|---|---|---|---|
| Android | `0.5.5`（versionCode `55`） | 正式版，已完成 OPPO PMX110 真机验收 | [`changelog/android.md`](changelog/android.md) |
...
给接手同事的入口。Android 已在 2026 年 9 月 1 日升至正式 `0.5.5`，收录列表稳定性修复和新版大图上滑删除动效，并通过 OPPO PMX110 真机验收；iOS 当前为正式 `0.5.5`，收录大图页真机修复、全局按压反馈，以及图片和参数信息的无感衔接优化。

### Git 状态

`main` 的正式基线包含 Android `0.5.5` 与 iOS `0.5.5` 的双端成果。安装包作为本地构建产物，不纳入 Git；换电脑后从源码重新构建即可。

### 安装包（本地构建输出，不纳入 Git）

| 平台 | 路径 | 说明 |
|---|---|---|
| Android | `APK/Punctum-0.5.5-release.apk` | 本地执行正式构建后生成；签名文件和密码需在新电脑安全迁移 |
...
**Android 0.5.5 正式版：** 包含 0.5.4 的首页图集卡和主要按钮按压反馈，并正式收录以下真机优化：

1. 大图上滑删除保持手势跟手，确认后沿连续轨迹上移并缩小到顶部红色删除按钮内；卡片边缘增加极淡描边。
2. 上滑有效触发距离缩短到约 101dp，删除确认动画为 300ms；删除按钮首次进入和连续删除时保持同一高度。
3. 背后的下一张照片提前承接，删除完成后不再从侧边滑入；列表长按删除继续立即补位，不附加淡出动效。
4. Camera 大图集的列表行身份保持稳定，并去除重复 URI，避免自动刷新时首张图片在第二格短暂重复。
5. 首页三种图集卡和主要导航、工具按钮继续使用按下下沉、松手回弹、回弹后执行的反馈。
6. 中文随机标题保留「观止，关心每一幅照片被重新看见的时刻」和「每一次回望，都重新感受影像的重量」。
7. 首页中文标题保持 Noto SemiBold、禁止描边；英文继续使用 Newsreader，票据与胶片排版规则保持 0.5.3 定稿。
8. 已在 OPPO PMX110 真机完成覆盖安装和交互验收，原有应用数据可保留。
