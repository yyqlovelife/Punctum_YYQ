...
- **顺手管理画廊**：长按图集整行即可拖动排序，列表固定高度并支持内部滚动、跨页自动滚动和右侧滚动提示。
...
图集排序弹窗支持长按拖动和触觉反馈；添加图集、选择图集和移动到图集的列表在内容超出范围时均保持固定高度，并显示滚动位置提示。

## 当前版本

README 按当前源码状态更新于 2026-09-04。Android 正式版本仍为 `0.5.6`，工作区包含等待下次发版收录的交互与视觉修复。

| 平台 | 版本 | 构建号 | 状态 |
|---|---:|---:|---|
| Android | `0.5.6` | `56` | 正式版；后续修复已在 OPPO PMA110 真机验证 |
